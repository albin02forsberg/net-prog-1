...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.userNameLbl = tk.Label(self.groupCon, text='Username', padx=10)
        self.userNameLbl.pack(side="left")

        self.userName = tk.Entry(self.groupCon, width=20)
        self.userName.insert(tk.END, 'User')
        self.userName.pack(side="left")

        self.userName.bind('<Return>', connectHandler)
        self.userName.pack(side="left")
        ...
        self.connectButton = tk.Button(self.groupCon,
            command = connectButtonClick, width=20)
        ...
        if incomingData.path == "/":
            if incomingData.data != None:
                for key in incomingData.data:
                    handleMessage(incomingData.data[key])
        else:
            handleMessage(incomingData.data)
...
def disconnect():
    ...
    global g_bConnected
    global g_sock
    g_app.connectButton['text'] = 'Connect with username'
